      ...
      <div className="max-w-md mx-auto bg-white rounded-t-3xl -mt-4 relative z-10 min-h-screen">
        <div className="p-8 text-center">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600 mb-1">
                  {localStorage.getItem('matchPreferences') ? JSON.parse(localStorage.getItem('matchPreferences')).runsPerWeek || '--' : '--'}
                </div>
                <div className="text-gray-600">Runs per week</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600 mb-1">
                  {localStorage.getItem('matchPreferences') ? JSON.parse(localStorage.getItem('matchPreferences')).longestRun || '--' : '--'}
                </div>
                <div className="text-gray-600">Longest run (miles)</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600 mb-1">
                  {localStorage.getItem('matchPreferences') ? JSON.parse(localStorage.getItem('matchPreferences')).myRunningStyle || '--' : '--'}
                </div>
                <div className="text-gray-600">Running style</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-600 mb-1">
                  {localStorage.getItem('matchPreferences') ? JSON.parse(localStorage.getItem('matchPreferences')).availability?.length || '--' : '--'}
                </div>
          ...
          <button
            ...
            className="w-full bg-orange-500 text-white py-4 rounded-lg font-bold text-lg hover:bg-orange-600 transition-colors shadow-lg"
          >
            🏃‍♂️ Go Find Matches
          </button>
